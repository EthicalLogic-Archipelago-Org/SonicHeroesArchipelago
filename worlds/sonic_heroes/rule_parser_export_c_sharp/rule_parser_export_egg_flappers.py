"""
Functions used by the parser related to EggFlappers (for exporting to C#)
"""
...
def get_egg_flappers_export_string_for_team(team: Team, parsed_data: dict[Stage, list[EggFlapper]]) -> str:
    _result: str = ""
    class_str: str = "EggFlapperData"
    list_name: str = "EggFlappers"
    tab_char: str = "    "
    ...
    for stage, parsed_list in parsed_data.items():
        for parsed_entry in parsed_list:
            params_dict: dict[str, str] = \
            {
                ...
                "weapon": f"EggFlapperWeapon.{parsed_entry.weapon.value.replace(" ", "")}",
                "armor": f"EggFlapperArmor.{parsed_entry.armor.value.replace(" ", "")}",
...
def get_egg_flappers_export_string() -> str:
    _result: str = ""
    full_parsed_data: dict[Team, dict[Stage, list[EggFlapper]]] = get_all_egg_flappers()
    for team in Team:
        print(f"Getting export string for {team}")
        _result += get_egg_flappers_export_string_for_team(team=team, parsed_data=full_parsed_data[team])
    return _result
