...
from worlds.sonic_heroes.constants.enemies import *
...
stage_obj_rule_mapping_dict: dict[str, SonicHeroesRuleFunctionMapping] = \
{
    ...
    "FloatingDice": SonicHeroesRuleFunctionMapping("has_floating_dice_obj"),

    #Enemy Objs
    "EggPawnNothing": SonicHeroesRuleFunctionMapping("can_egg_pawn"),
    "KillEggPawnNothing": SonicHeroesRuleFunctionMapping("can_kill_egg_pawn"),
    ...
    "EggPawnNoWeapon": SonicHeroesRuleFunctionMapping("has_egg_pawn_obj", {"weapon": EggPawnWeapon.NO_WEAPON}),
...
def does_start_and_end_parens_match(rule: str) -> bool:
    if not (rule[0] == '(' and rule[-1] == ')'):
        return False
    paren_match: int = 0
...
def handle_rule(rule: str, print_steps: bool = False):
    ...
    if rule[0] == '(' and rule[-1] == ')' and does_start_and_end_parens_match(rule):
...
def handle_full_rule_string(rule: str, print_steps: bool = False) -> str:
    ...
    individual_rule_team, individual_rule_level = "Dark", "Seaside Hill"
...
#test_rule = "BreakThingsOR((EggPawnNoWeaponOREggPawnBazooka)ANDHeightJumpANDHoming0)ORHeightFlyOR((KillEggPawnNoWeaponANDKillEggPawnBazooka)ANDHeightJumpANDTornadoHover)OR(HeightFlyNoJumpSoloJumpANDParkour)OR(HeightJumpANDThundershoot)"
#print(handle_full_rule_string(test_rule, True))

test_rule3 = "((FloatingDiceANDRegularSwitch)ORRegularWeight)AND(FlyingAnyANDPushPullSwitch)"
print(handle_full_rule_string(test_rule3, True))

#test_rule4 = "BreakThingsOR(EggPawnNothingANDHoming0)OR(FlyingAny)OR(KillEggPawnNothingANDTornadoRegular0)ORParkour"
#print(handle_full_rule_string(test_rule4, True))

#test_rule5 = "((BreakThingsAND((FlyingOneCharANDHeightJump)ORFlyingFull)OR(FlyingAnyANDParkour))ANDJump)OR(FlyingAnyANDFloatingDice)"
#print(handle_full_rule_string(test_rule5, True))
