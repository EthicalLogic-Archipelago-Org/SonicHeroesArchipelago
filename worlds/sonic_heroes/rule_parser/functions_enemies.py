...
def parse_enemy_csv(team: Team, stage: Stage, secret: bool = False) -> None:
    ...
    with files(csv_data.csv_data_mapping[stage][team]).joinpath(f"{file_name}.csv").open() as csv_file:  # pyright: ignore[reportUnknownMemberType, reportUnknownVariableType]
        ...
        for x in reader:
            ...
            enemy_type_str: str = x[TYPE_HEADER]
            weapon_str: str = x[WEAPON_HEADER]
            armor_str: str = x[ARMOR_HEADER]
            shield_str: str = x[SHIELD_HEADER]
            height_str: str = x[HEIGHT_HEADER]
            x_coord: float = float(x[X_HEADER])
            y_coord: float = float(x[Y_HEADER])
            z_coord: float = float(x[Z_HEADER])
            ...
            # EggFlapper(armor=EggFlapperArmor.NO_ARMOR, weapon=EggFlapperWeapon.NO_WEAPON, height=EnemyHeight.GROUND)

            # enemy_str_list.append(f"ItemBalloonBoxData(team={team_str}, stage={stage_str}, region_name=\"{region_name}\", item={item_str}, x={float(x[X_HEADER])}, y={float(x[Y_HEADER])}, z={float(x[Z_HEADER])}, rule={parsed_rule_str})")


    parsed_result: str = f"\n{ENEMY_PARSER_FILE_HEADER}\n{stage.stage_name.replace(" ", "")}{team.replace(" ", "")}Enemies: list[SonicHeroesEnemy] = \\\n[\n    {',\n    '.join(enemy_str_list)}\n]"
