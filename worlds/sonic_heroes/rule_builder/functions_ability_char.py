...
def can_jump_rule(team: Team, stage: Stage) -> Rule[SonicHeroesWorldBase]:
    ...
    return SonicHeroesMacroRule(child=HasAbilityItem(ability=ability, team=team, stage=stage), name=f"{ability.ability_name} as Team: {team} in {stage.stage_name}")


def can_amy_hammer_hover_rule(team: Team, stage: Stage) -> Rule[SonicHeroesWorldBase]:
    ability: Ability = Ability.AMY_HAMMER_HOVER
    return SonicHeroesMacroRule(child=HasAbilityItem(ability=ability, team=team, stage=stage) & can_jump_rule(team=team, stage=stage), name=f"{ability.ability_name} as Team: {team} in {stage.stage_name}", description=f"Hover in the air by spinning Amy's Hammer. Hold the jump button after jumping")
...
def can_kick(team: Team, stage: Stage) -> Rule[SonicHeroesWorldBase]:
...
def can_rocket_accel_rule(team: Team, stage: Stage, num_other_chars: int) -> Rule[SonicHeroesWorldBase]:
    ...
    return SonicHeroesMacroRule(child=HasAbilityItemLevelOtherChars(ability=ability, team=team, stage=stage, level=0, num_other_chars=num_other_chars), name=f"{ability.ability_name} with {num_other_chars} other characters as Team: {team} in {stage.stage_name}")
...
def can_triangle_jump_rule(team: Team, stage: Stage) -> Rule[SonicHeroesWorldBase]:
    ...
    return SonicHeroesMacroRule(child=HasAbilityItem(ability=ability, team=team, stage=stage) & can_homing_attack_rule(team=team, stage=stage, level=0) & can_jump_rule(team=team, stage=stage), name=f"{ability.ability_name} as Team: {team} in {stage.stage_name}")


def can_light_attack_rule(team: Team, stage: Stage) -> Rule[SonicHeroesWorldBase]:
    ability: Ability = Ability.LIGHT_ATTACK
    return SonicHeroesMacroRule(child=HasAbilityItem(ability=ability, team=team, stage=stage) & can_jump_rule(team=team, stage=stage) & can_team_blast_rule(team=team, stage=stage), name=f"{ability.ability_name} as Team: {team} in {stage.stage_name}")


def can_invisibility_rule(team: Team, stage: Stage) -> Rule[SonicHeroesWorldBase]:
    ability: Ability = Ability.INVISIBILITY
    return SonicHeroesMacroRule(child=HasAbilityItem(ability=ability, team=team, stage=stage) & can_jump_rule(team=team, stage=stage) & can_tornado_rule(team=team, stage=stage, level=0), name=f"{ability.ability_name} as Team: {team} in {stage.stage_name}")
...
def can_thundershoot_rule(team: Team, stage: Stage, level: int) -> Rule[SonicHeroesWorldBase]:
    ...
    return SonicHeroesMacroRule(child=HasAbilityItemLevelOtherChars(ability=ability, team=team, stage=stage, level=level, num_other_chars=1), name=f"{ability.ability_name} Level {level} as Team: {team} in {stage.stage_name}")
...
def can_auto_power_attack_rule(team: Team, stage: Stage, need_speed_lvl_3: bool = False) -> Rule[SonicHeroesWorldBase]:
    rule: Rule[SonicHeroesWorldBase] = CanAutoPowerAttack(team=team, stage=stage, need_speed_lvl_3=need_speed_lvl_3)
...
def can_power_attack_rule(team: Team, stage: Stage, level: int) -> Rule[SonicHeroesWorldBase]:
    ...
    return SonicHeroesMacroRule(child=HasAbilityItemLevel(ability=ability, team=team, stage=stage, level=level), name=f"{ability.ability_name} Level {level} as Team: {team} in {stage.stage_name}")
...
def can_fire_dunk_rule(team: Team, stage: Stage, level: int) -> Rule[SonicHeroesWorldBase]:
    ...
    return SonicHeroesMacroRule(child=HasAbilityItemLevelOtherChars(ability=ability, team=team, stage=stage, level=level, num_other_chars=1) & can_jump_rule(team=team, stage=stage), name=f"{ability.ability_name} Level {level} as Team: {team} in {stage.stage_name}")
...
def can_ultimate_fire_dunk_rule(team: Team, stage: Stage, level: int) -> Rule[SonicHeroesWorldBase]:
    ...
    return SonicHeroesMacroRule(child=HasAbilityItemLevelOtherChars(ability=ability, team=team, stage=stage, level=level, num_other_chars=1) & can_fire_dunk_rule(team=team, stage=stage, level=level) & can_jump_rule(team=team, stage=stage), name=f"{ability.ability_name} Level {level} as Team: {team} in {stage.stage_name}")


def can_glide_rule(team: Team, stage: Stage) -> Rule[SonicHeroesWorldBase]:
    ability: Ability = Ability.GLIDE
    return SonicHeroesMacroRule(child=HasAbilityItem(ability=ability, team=team, stage=stage) & can_jump_rule(team=team, stage=stage), name=f"{ability.ability_name} as Team: {team} in {stage.stage_name}")
...
def can_combo_finisher_rule(team: Team, stage: Stage, level: int) -> Rule[SonicHeroesWorldBase]:
    ...
    return SonicHeroesMacroRule(child=HasAbilityItemLevel(ability=ability, team=team, stage=stage, level=level) & can_power_attack_rule(team=team, stage=stage, level=level), name=f"{ability.ability_name} Level {level} as Team: {team} in {stage.stage_name}")


def can_combo_height_rule(team: Team, stage: Stage) -> Rule[SonicHeroesWorldBase]:
    return SonicHeroesMacroRule(child=HasComboHeight(team=team, stage=stage) & can_power_attack_rule(team=team, stage=stage, level=0), name=f"Gain Height with {Ability.COMBO_FINISHER.ability_name} as Team: {team} in {stage.stage_name}")


def can_team_blast_rule(team: Team, stage: Stage) -> Rule[SonicHeroesWorldBase]:
    return CanTeamBlast(team=team, stage=stage)
