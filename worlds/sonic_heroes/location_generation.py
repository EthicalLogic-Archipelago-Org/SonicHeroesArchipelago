...
def append_location(name: str, team: Team, stage: Stage, code: int, act: int, parent_region: str, rule_str: str, rule: Rule[SonicHeroesWorldBase], loc_type: LocationType, locked_item: str = "", num_to_increment_id: int = 1) -> None:
    global loc_id
    if code > 0:
        loc_id = code
    code = loc_id
...
def generate_emerald_locations() -> None:
    ...
    for reg_lvl, bonus_lvl in Stage.get_stage_to_bonus_stage().items():
        if "Emerald Stage" in bonus_lvl.stage_name:
            append_location(name=f"{reg_lvl.stage_name} Emerald", team=Team.ANY_TEAM, stage=bonus_lvl, code=-999, act=0, parent_region=f"{bonus_lvl.stage_name}", rule_str="", rule=CanGetEmerald(stage=bonus_lvl), loc_type=LocationType.EMERALD)
...
def generate_rose_obj_sanity() -> None:
    ...
    for reg_lvl in Stage.get_stages_of_type(stage_type=StageType.NORMAL_STAGE):
        for x in range(200):
            append_location(name=f"{reg_lvl.stage_name} {Team.ROSE} {Act.ACT_2} Rings Collected: {x + 1}", team=Team.ROSE, stage=reg_lvl, code=-999, act=2, parent_region=f"{reg_lvl} {Team.ROSE} ObjSanity", rule_str="", rule=Has(item_name=get_obj_sanity_event_item_name(team=Team.ROSE, stage=reg_lvl, act=Act.ACT_2), count=x + 1), loc_type=LocationType.OBJ_SANITY)
...
def generate_checkpoint_sanity_super_hard_mode() -> None:
    ...
    for reg_lvl in Stage.get_stages_of_type(stage_type=StageType.NORMAL_STAGE):
        for x in range(reg_lvl.checkpoints[team]):
            append_location(name=f"{reg_lvl.stage_name} {team} Checkpoint {x + 1}", team=team, stage=reg_lvl, code=-999, act=2, parent_region=f"{reg_lvl.stage_name} {team} Checkpoint {x + 1}", rule_str=f"Checkpoint", rule=has_stage_obj_rule(stage_obj=StageObj.CHECKPOINT), loc_type=LocationType.CHECKPOINT_SANITY)
...
def generate_level_goal_locations_for_super_hard_mode_hard_mode_goals() -> None:
    ...
    team: Team = Team.SUPER_HARD_MODE
    for reg_lvl in Stage.get_stages_of_type(stage_type=StageType.NORMAL_STAGE):
        append_location(name=f"{reg_lvl.stage_name} {team}", team=team, stage=reg_lvl, code=-999, act=2, parent_region=f"{reg_lvl.stage_name} {team} Goal", rule_str=f"GoalRing", rule=CanGoalStage(team=team, stage=reg_lvl), loc_type=LocationType.LEVEL)
...
def generate_metal_madness_extra_locations() -> None:
    ...
    append_location(name=f"{Stage.METAL_OVERLORD.stage_name}", team=Team.ANY_TEAM, stage=Stage.METAL_MADNESS, code=-999, act=0, parent_region=f"{Stage.METAL_MADNESS.stage_name}", rule_str=f"", rule=True_[SonicHeroesWorldBase](), loc_type=LocationType.LEVEL)
...
def generate_boss_all_teams_events() -> None:
    for boss in Stage.get_stages_of_type(stage_type=StageType.BOSS_STAGE):
        append_location(name=f"{boss.stage_name} {EVENT_LOCATION}", team=Team.ANY_TEAM, stage=boss, code=EVENT_LOCATION_ID, act=0, parent_region=f"{boss.stage_name}", rule_str="", rule=True_[SonicHeroesWorldBase](), loc_type=LocationType.EVENT, locked_item=f"{boss.stage_name} {EVENT_ITEM}")
...
def generate_level_goal_all_teams_events() -> None:
    for team in Team:
        if team == Team.ANY_TEAM:
            ...
            continue
        for reg_lvl in Stage.get_stages_of_type(stage_type=StageType.NORMAL_STAGE):
            append_location(name=f"{reg_lvl.stage_name} {team} Goal {EVENT_LOCATION}", team=team, stage=reg_lvl, code=EVENT_LOCATION_ID, act=0, parent_region=f"{reg_lvl.stage_name} {team} Goal", rule_str=f"", rule=CanGoalStage(team=team, stage=reg_lvl), loc_type=LocationType.EVENT, locked_item=LEVEL_GOAL_ALL_TEAMS_EVENT_ITEM)
